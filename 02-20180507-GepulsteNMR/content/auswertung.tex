...
\begin{align*}
  U &= a \cdot \exp{\left(- \tau^3 \cdot d\right)} \cdot c \cdot \exp{\left(-b \frac{\tau}{T_2}\right)}+ m \\
  \intertext{ergibt die Parameter}
...
\begin{align*}
  r_\text{Molekülradius} &= {\left(\frac{M}{\rho N_A}\right)}^{\sfrac{1}{3}} =
    ...
    \hspace{1em}\text{, mit } V_\text{krit.} = b \hspace{1em}\text{, $b$ VdW-Kovolumen\cite{vdw_radius}}\\
